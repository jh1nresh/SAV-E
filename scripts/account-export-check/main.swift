...
@MainActor final class PrivyAuthService {
    static let shared = PrivyAuthService()
    var isReviewerDemo = false
    var currentUserId: String? = "did:privy:fixture"
    var sessionGeneration = 0
}
enum SupabaseError: Error { case apiError(Int, String), notAuthenticated }
...
@main struct AccountExportCheck {
    @MainActor static func main() async throws {
        ...
        var calls: [String] = []
        let first = try await DebugVaultExporter.export(to: root, source: source, accountSubject: "did:privy:fixture") { path in
            calls.append(path); return response(path)
        }
        let good = try manifest(first)
        check(good.accountSubjectSHA256 == SHA256.hash(data: Data("did:privy:fixture".utf8)).map { String(format: "%02x", $0) }.joined(), "same-session identity proof")
        ...
        let second = try await DebugVaultExporter.export(to: root, source: source, accountSubject: "did:privy:fixture") { path in
        ...
        calls = []
        let viewer = try await DebugVaultExporter.export(to: root, source: source, accountSubject: "did:privy:fixture") { path in
        ...
        let denied = try await DebugVaultExporter.export(to: root, source: source, accountSubject: "did:privy:fixture") { path in
        ...
        for badResponse in ["not json", "{\"error\":\"failed\"}", "[1,2]"] {
            let dir = try await DebugVaultExporter.export(to: root, source: source, accountSubject: "did:privy:fixture") { path in
        ...
        var profiles = 0
        let changed = try await DebugVaultExporter.export(to: root, source: source, accountSubject: "did:privy:fixture") { path in
            if path == "/profile" {
                profiles += 1
        ...
        calls = []
        let signedOut = try await DebugVaultExporter.export(to: root, source: source, accountSubject: "did:privy:fixture") { path in
        ...
        let malformedList = try await DebugVaultExporter.export(to: root, source: source, accountSubject: "did:privy:fixture") { path in
        ...
        let emptyIdentity = try await DebugVaultExporter.export(to: root, source: source, accountSubject: "did:privy:fixture") { path in
        ...
        let duplicate = try await DebugVaultExporter.export(to: root, source: source, accountSubject: "did:privy:fixture") { path in
            if path == "/v0/lists" {
        ...
        for invalid in ["http://wanderly-api-production.up.railway.app", "https://evil.example", "https://wanderly-api-production.up.railway.app.evil.example", "https://user@wanderly-api-production.up.railway.app", source + "/path", source + "?query=x"] {
            ...
            do {
                _ = try await DebugVaultExporter.export(to: root, source: invalid, accountSubject: "did:privy:fixture") { _ in readCalled = true; return empty }
        ...
        let unsupported = try await DebugVaultExporter.export(to: root, source: source, accountSubject: "did:privy:fixture") { path in
            if path == "/v0/shared-posts/mine" { throw SupabaseError.apiError(404, "PRIVATE ERROR BODY") }
            if path == "/v0/social-profile" { throw SupabaseError.notAuthenticated }
            return response(path)
        }
        let diagnostics = try manifest(unsupported)
        check(diagnostics.serviceChecks.first?.httpStatus == 404, "old API missing route must be distinguishable")
